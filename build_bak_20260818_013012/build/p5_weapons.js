...
const Weapons = {
  _owner: null,          // 双人：本武器集归属的玩家（P1=Player，P2=Player2），由 Game.start 装配
  currentTarget: null,
  t: 0,
  cd: { cannon: 0, missile: 0, laser: 0, aura: 0, spread: 0, chain: 0, drone: 0, nova: 0, saw: 0, rail: 0, flame: 0, pulse: 0, frost: 0, meteor: 0, swarm: 0, storm: 0,
        ...
        radial: 0, reflect: 0, lance: 0, pulsar: 0, siege: 0, scatter: 0, ion: 0 },
  ...
  TABLE: {
    ...
    /* 径向脉冲：周期性向四周喷射环形弹幕，无死角清场（E 新增，tag=barrage） */
    // 环形弹幕：弹数收敛(原 10→20 过密压 InstancedMesh)，总伤基本持平
    radial: [
      { n:8,  dmg:11, cd:1.50 },
      { n:10, dmg:13, cd:1.35 },
      { n:12, dmg:15, cd:1.20 },
    ...
    /* 锋芒立场：环绕自身的反射力场，弹开敌弹并周期灼烧近身敌人（E 新增，tag=heavy） */
    reflect: [
      { r:4.2, dmg:10, tick:0.28 },
      { r:4.8, dmg:13, tick:0.25 },
      { r:5.4, dmg:16, tick:0.22 },
      { r:6.0, dmg:20, tick:0.20 },
      { r:6.8, dmg:25, tick:0.18 },
    ],
    /* 蓄能长矛：蓄力后射出撕穿全场的穿透重矛（E 新增，tag=precise/heavy） */
    lance: [
      { dmg:55,  cd:1.70, spd:130, pierce:999, charge:0.45 },
      { dmg:72,  cd:1.55, spd:135, pierce:999, charge:0.42 },
      { dmg:92,  cd:1.40, spd:140, pierce:999, charge:0.40 },
      { dmg:116, cd:1.25, spd:148, pierce:999, charge:0.38 },
      { dmg:145, cd:1.10, spd:156, pierce:999, charge:0.35 },
    ],
    /* 脉冲星斩舰炮：蓄能后射出贯穿脉冲束，沿途敌人高伤并被击退（E+，tag=precise/energy）
       蓄力/冷却略收紧，手感更顺 */
    pulsar: [
      { dmg:60,  cd:2.0, charge:0.60, spd:120 },
      { dmg:78,  cd:1.85, charge:0.58, spd:130 },
      { dmg:100, cd:1.70, charge:0.55, spd:140 },
      { dmg:128, cd:1.55, charge:0.52, spd:150 },
      { dmg:165, cd:1.40, charge:0.50, spd:160 },
    ],
    /* 攻城轨道炮：蓄能后锁定最远敌人，落点大范围爆破（E+，tag=heavy/precise）
       重型炮，蓄力/冷却略收紧但保持厚重感 */
    siege: [
      { dmg:90,  cd:2.6, charge:0.90, spd:90, splash:7, r:8 },
      { dmg:120, cd:2.4, charge:0.85, spd:95, splash:9, r:9 },
      { dmg:155, cd:2.2, charge:0.80, spd:100, splash:11, r:10 },
      { dmg:200, cd:2.0, charge:0.75, spd:105, splash:13, r:11 },
      { dmg:260, cd:1.8, charge:0.70, spd:110, splash:15, r:12 },
    ],
    /* 离子散射炮：一次扇形发射多枚高速离子弹清场（E+，tag=barrage/energy）
       弹数 +1 提升清场覆盖 */
    scatter: [
      { dmg:14, cd:0.9, n:6, spd:60 },
      { dmg:17, cd:0.85, n:7, spd:64 },
      { dmg:21, cd:0.8, n:8, spd:68 },
      { dmg:26, cd:0.75, n:9, spd:72 },
      { dmg:32, cd:0.7, n:10, spd:76 },
    ],
    /* 离子干扰炮：三连发离子弹，命中使敌人减速（E+，tag=precise/energy） */
    ion: [
      { dmg:22, cd:1.1, n:3, spd:70, slow:1.6, slowK:0.5 },
      { dmg:28, cd:1.0, n:3, spd:74, slow:1.8, slowK:0.5 },
      { dmg:35, cd:0.9, n:4, spd:78, slow:2.0, slowK:0.45 },
      { dmg:44, cd:0.8, n:4, spd:82, slow:2.2, slowK:0.45 },
      { dmg:55, cd:0.7, n:5, spd:86, slow:2.4, slowK:0.4 },
    ],
  },
  ...
  init(){
    ...
    for (let i = 0; i < 5; i++){
      ...
      this.drones.push({ mesh: g, a: 0, cd: 0, r: 5.0, px: this._owner.x, pz: this._owner.z, recoil: 0 });
    }
    this.droneT = 0;
    ...
    // —— E 新武器常驻网格 ——
    // 锋芒立场：环绕玩家的反射力场环（青色，常驻显示）
    const rg = new THREE.RingGeometry(0.7, 1, 48);
    this.reflectMesh = new THREE.Mesh(rg, new THREE.MeshBasicMaterial({
      color: 0x8ff0ff, transparent: true, opacity: 0,
      side: THREE.DoubleSide, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }));
    this.reflectMesh.rotation.x = -Math.PI / 2; this.reflectMesh.position.y = 0.12;
    this.reflectMesh.visible = false; World.scene.add(this.reflectMesh);

    // 蓄能长矛：蓄力时的瞄准/充能指示（细圆柱，沿 +Z，原点在底座）
    const lg = new THREE.CylinderGeometry(0.18, 0.5, 1, 10, 1, true);
    lg.rotateX(Math.PI / 2); lg.translate(0, 0, 0.5);
    this.lanceMesh = new THREE.Mesh(lg, new THREE.MeshBasicMaterial({
      color: 0xbfe9ff, transparent: true, opacity: 0,
      side: THREE.DoubleSide, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }));
    this.lanceMesh.visible = false; World.scene.add(this.lanceMesh);
  },
  ...
  reset(){
    ...
    // E 新武器状态清理
    if (this.reflectMesh) this.reflectMesh.visible = false;
    if (this.lanceMesh) this.lanceMesh.visible = false;
    this.lanceCharging = false; this.lanceChargeT = 0; this.lanceAim = 0;
    this.pulsarCharging = false; this.pulsarChargeT = 0; this.pulsarAim = 0;
    this.siegeCharging = false; this.siegeChargeT = 0; this.siegeAim = 0;
    this.reflectTick = 0;
  },
  ...
  acquire(){
    if (Boss.active && !Boss.entering){
      const d = Math.hypot(Boss.x - this._owner.x, Boss.z - this._owner.z);
      if (d < 54){ this.currentTarget = Boss.asTarget(); return; }
    }
    const t = this.nearestTo(this._owner.x, this._owner.z, 46);
    this.currentTarget = t;
  },

  // 基础射速倍率 + 能量核心过载(overdrive) 临时提速 40%
  rateMul(){ return 1 / (1 + this._owner.progress.p('rate') * 0.09) * (this._owner.buffs && this._owner.buffs.overdrive > 0 ? 0.6 : 1); },
  rollCrit(){ return Math.random() < Math.min(0.9, this._owner.progress.p('crit') * 0.07 + Synergy.mods.crit); },
  critMul(){ return 2.1 + this._owner.progress.p('crit') * 0.05 + Synergy.mods.critDmg; },
  ...
  update(dt){
    ...
    const rm = this.rateMul() / (this._owner.fireMul || 1);

    /* ---------- 主炮 ---------- */
    const cl = this._owner.progress.w('cannon');
    if (cl > 0){
      this.cd.cannon -= dt;
      if (this.cd.cannon <= 0 && T){
        const S = this.TABLE.cannon[cl - 1];
        this.cd.cannon = S.cd * rm;
        const base = Math.atan2(T.x - this._owner.x, T.z - this._owner.z);
        for (let i = 0; i < S.n; i++){
          ...
          const px = this._owner.x + Math.cos(base) * side + Math.sin(base) * 1.2;
          const pz = this._owner.z - Math.sin(base) * side + Math.cos(base) * 1.2;
          Bullets.fire(px, pz, base + off, 62, dmg, {
            pierce: S.pierce, crit, long: 1.3,
            color: crit ? 0xffcc33 : 0x9df6ff, life: 1.3 });
        }
        const mx = this._owner.x + Math.sin(base) * 1.5, mz = this._owner.z + Math.cos(base) * 1.5;
    ...
    /* ---------- 追踪导弹 ---------- */
    const ml = this._owner.progress.w('missile');
    if (ml > 0){
      this.cd.missile -= dt;
      if (this.cd.missile <= 0 && T){
        ...
        for (let i = 0; i < S.n; i++){
          const crit = this.rollCrit();
          const a = this._owner.yaw + Math.PI + (i - (S.n - 1) / 2) * 0.7 + Util.rand(-0.15, 0.15);
          Bullets.missile(this._owner.x + Math.sin(a) * 0.8, this._owner.z + Math.cos(a) * 0.8,
            S.dmg * (crit ? this.critMul() : 1),
            (i === 0 ? T : this.nearestTo(this._owner.x, this._owner.z, 46)) || T,
            { dir: a, splash: S.splash, crit, spd: 12, scale: 1 + ml * 0.06 });
        }
        FX.ring(this._owner.x, this._owner.z, 0xffb24a, 2.6, 0.2);   // 导弹发射闪光
        Audio2.missile();
      }
    }

    /* ---------- 相位激光 ---------- */
    const ll = this._owner.progress.w('laser');
    if (ll > 0){
      this.cd.laser -= dt;
    ...
    if (al > 0){
      ...
      if (this.cd.aura <= 0){
        ...
        const hits = Enemies.queryHit(this._owner.x, this._owner.z, S.r);
        for (const e of hits){
          const crit = this.rollCrit();
        ...
        if (hits.length){
          FX.ring(this._owner.x, this._owner.z, 0xb980ff, S.r * 1.05, 0.3);
          // 力场内缘向心电弧，强化"湮灭力场"观感
          for (let k = 0; k < 7; k++){
            const a = Math.random() * Util.TAU;
            FX.particle(this._owner.x + Math.cos(a) * S.r, 1.0, this._owner.z + Math.sin(a) * S.r,
              0xb980ff, { life: 0.3, s0: 0.5, s1: 0.1, drag: 2,
                vx: -Math.cos(a) * 5, vz: -Math.sin(a) * 5 });
          }
        }
        if (Boss.active && Boss.hitTest(this._owner.x, this._owner.z, S.r))
    ...
    /* ---------- 散射霰弹 ---------- */
    const sl = this._owner.progress.w('spread');
    if (sl > 0 && T){
      this.cd.spread -= dt;
      if (this.cd.spread <= 0){
        ...
        for (let i = 0; i < S.n; i++){
          ...
          Bullets.fire(this._owner.x + Math.sin(base) * 1.1, this._owner.z + Math.cos(base) * 1.1,
            base + off, 54, S.dmg * (crit ? this.critMul() : 1),
            { crit: crit && i === (S.n >> 1), y: 1.0, color: 0xe7ffb0, scale: 0.7, life: 0.7 });
        }
        const sx = this._owner.x + Math.sin(base) * 1.4, sz = this._owner.z + Math.cos(base) * 1.4;
    ...
    /* ---------- 环绕光刃 ---------- */
    const ol = this._owner.progress.w('orbit');
    this.orbitT += dt;
    if (ol > 0){
      ...
      for (let i = 0; i < this.blades.length; i++){
        ...
        if (i < S.n){
          ...
          const bx = this._owner.x + Math.cos(a) * S.r;
          const bz = this._owner.z + Math.sin(a) * S.r;
    ...
    /* ---------- 连锁闪电 ---------- */
    const chl = this._owner.progress.w('chain');
    if (chl > 0){
      this.cd.chain -= dt;
      if (this.cd.chain <= 0 && T){
        const S = this.TABLE.chain[chl - 1];
        this.cd.chain = S.cd * rm;
        const pts = [{x: this._owner.x, z: this._owner.z}];
        let cur = T, prev = null;
        ...
        if (pts.length > 1){ Audio2.chainZap(); FX.ring(this._owner.x, this._owner.z, 0x9df6ff, 4.2, 0.28); }
      }
    }

    /* ---------- 无人僚机 ---------- */
    const dl = this._owner.progress.w('drone');
    this.droneT += dt;
    if (dl > 0){
      ...
      for (let i = 0; i < this.drones.length; i++){
        ...
        if (i < S.n){
          ...
          const rad = S.r + wob;
          const tx = this._owner.x + Math.cos(d.a) * rad;
          const tz = this._owner.z + Math.sin(d.a) * rad;
    ...
    /* ---------- 湮灭新星 ---------- */
    const nl = this._owner.progress.w('nova');
    if (nl > 0){
      this.cd.nova -= dt;
      if (this.cd.nova <= 0 && (Enemies.pool.active.length > 0 || Boss.active)){
        const S = this.TABLE.nova[nl - 1];
        this.cd.nova = S.cd * rm;
        FX.ring(this._owner.x, this._owner.z, 0xff5d8a, S.r, 0.5);
        FX.ring(this._owner.x, this._owner.z, 0xffd24a, S.r * 0.62, 0.36);
        FX.ring(this._owner.x, this._owner.z, 0xffffff, S.r * 0.3, 0.22);   // 核心白闪
        FX.burst(this._owner.x, this._owner.z, 0xff5d8a, 16, 8, 1.4);        // 星爆射线
        FX.burst(this._owner.x, this._owner.z, 0xffffff, 7, 4.5, 1.4);       // 核心白芯
        const hits = Enemies.queryHit(this._owner.x, this._owner.z, S.r);
        for (const e of hits){
          const crit = this.rollCrit();
          Enemies.damage(e, S.dmg * (crit ? this.critMul() : 1), crit, e.x, e.z);
    ...
    /* ---------- 回旋飞锯 ---------- */
    const wl = this._owner.progress.w('saw');
    if (wl > 0){
      this.cd.saw -= dt;
      if (this.cd.saw <= 0 && T){
        ...
        for (let i = 0; i < this.saws.length && thrown < S.n; i++){
          ...
          s.x = this._owner.x; s.z = this._owner.z;
    ...
    for (const s of this.saws){
      ...
      if (s.returning){
        ...
        const dx = this._owner.x - s.x, dz = this._owner.z - s.z, d = Math.hypot(dx, dz);
        const rs = 30;
    ...
    /* ---------- 电磁轨道炮 ---------- */
    const rl = this._owner.progress.w('rail');
    if (rl > 0){
      this.cd.rail -= dt;
      if (this.cd.rail <= 0 && T){
        const S = this.TABLE.rail[rl - 1];
        this.cd.rail = S.cd * rm;
        const dir = Math.atan2(T.x - this._owner.x, T.z - this._owner.z);
        const crit = this.rollCrit();
        Bullets.fire(this._owner.x, this._owner.z, dir, S.spd, S.dmg * (crit ? this.critMul() : 1),
          { pierce: S.pierce, crit, color: crit ? 0xffcc33 : 0x9df6ff, scale: 1.4, life: 2.4, long: 3.2, y: 1.0 });
        FX.ring(this._owner.x, this._owner.z, 0x9df6ff, 3, 0.18);
        Audio2.rail();
      }
    }

    /* ---------- 烈焰喷射 ---------- */
    const fl = this._owner.progress.w('flame');
    if (fl > 0){
      this.cd.flame -= dt;
      if (this.cd.flame <= 0){
        const S = this.TABLE.flame[fl - 1];
        this.cd.flame = S.cd * rm;
        const base = T ? Math.atan2(T.x - this._owner.x, T.z - this._owner.z) : this._owner.yaw;
        const list = Enemies.pool.active;
        for (let i = 0; i < list.length; i++){
          const e = list[i]; if (!e.alive) continue;
          const dx = e.x - this._owner.x, dz = e.z - this._owner.z;
        ...
        if (Boss.active && !Boss.entering){
          const dx = Boss.x - this._owner.x, dz = Boss.z - this._owner.z, d = Math.hypot(dx, dz);
          if (d <= S.range + Boss.r){
            const ang = Math.atan2(dx, dz);
            const da = Math.abs(((ang - base + Math.PI) % Util.TAU) - Math.PI);
            if (da <= S.arc) Boss.damage(S.dmg, false, this._owner.x, this._owner.z);
          }
        }
        if (T){
          for (let k = 0; k < 9; k++){
            ...
            FX.particle(this._owner.x + Math.sin(a) * r, 1.0 + Math.random() * 0.6,
              this._owner.z + Math.cos(a) * r, Util.pick([0xff9a3d, 0xffd24a, 0xff5a2f]),
    ...
    /* ---------- 声波脉冲 ---------- */
    const pl = this._owner.progress.w('pulse');
    if (pl > 0){
      this.cd.pulse -= dt;
      if (this.cd.pulse <= 0){
        const S = this.TABLE.pulse[pl - 1];
        this.cd.pulse = S.cd * rm;
        this.pulses.push({ x: this._owner.x, z: this._owner.z, t: 0, life: 0.6, maxR: S.r, dmg: S.dmg, hit: new Set(), col: 0x39e0ff });
        FX.ring(this._owner.x, this._owner.z, 0x39e0ff, S.r, 0.6);
        Audio2.pulse();
      }
    }
    ...
    const fr = this._owner.progress.w('frost');
    if (fr > 0){
      this.cd.frost -= dt;
      if (this.cd.frost <= 0 && T){
        ...
        for (let i = 0; i < S.n; i++){
          ...
          const crit = this.rollCrit();
          const dmg = S.dmg * (crit ? this.critMul() : 1);
          Bullets.fire(this._owner.x + Math.sin(base) * 1.2, this._owner.z + Math.cos(base) * 1.2,
    ...
    const mt = this._owner.progress.w('meteor');
    if (mt > 0){
      this.cd.meteor -= dt;
      if (this.cd.meteor <= 0){
        const S = this.TABLE.meteor[mt - 1];
        this.cd.meteor = S.cd * rm;
        let tx = T ? T.x : this._owner.x, tz = T ? T.z : this._owner.z;
        if (!T){ const near = this.nearestTo(this._owner.x, this._owner.z, 60); if (near){ tx = near.x; tz = near.z; } }
    ...
    const sw = this._owner.progress.w('swarm');
    if (sw > 0){
      this.cd.swarm -= dt;
      if (this.cd.swarm <= 0 && T){
        ...
        for (let i = 0; i < S.n; i++){
          const crit = this.rollCrit();
          const tg = tgs[i] || T;
          const a = this._owner.yaw + Math.PI + (i / S.n) * Util.TAU + Util.rand(-0.1, 0.1);
          Bullets.missile(this._owner.x, this._owner.z, S.dmg * (crit ? this.critMul() : 1), tg,
    ...
    const st = this._owner.progress.w('storm');
    if (st > 0){
      this.cd.storm -= dt;
    ...
    // 黑洞：生成引力井，吸附+灼烧，到期坍缩
    const bh = this._owner.progress.w('blackhole');
    if (bh > 0){
      ...
      if (this.cd.blackhole <= 0 && (Enemies.pool.active.length > 0 || Boss.active)){
        ...
        let bx = T ? T.x : this._owner.x, bz = T ? T.z : this._owner.z;
        if (!T){ const near = this.nearestTo(this._owner.x, this._owner.z, 60); if (near){ bx = near.x; bz = near.z; } }
        const g = new THREE.Group();
    ...
    // 相位护盾：周期展开吸收护盾
    const ph = this._owner.progress.w('phase');
    if (ph > 0){
      this.cd.phase -= dt;
      if (this.cd.phase <= 0){
        const S = this.TABLE.phase[ph - 1];
        this.cd.phase = S.cd * rm;
        this._owner.phaseHp = S.hp; this._owner.phaseMax = S.hp; this._owner.phaseT = S.dur;
        FX.ring(this._owner.x, this._owner.z, 0x38f0ff, 3.0, 0.4);
        Audio2.phase();
      }
      if (this._owner.phaseT > 0) this._owner.phaseT -= dt;
      else this._owner.phaseHp = 0;
    }

    // 光子跳弹：弹射贯穿
    const pt = this._owner.progress.w('photon');
    if (pt > 0){
      this.cd.photon -= dt;
      if (this.cd.photon <= 0 && T){
        ...
        for (let i = 0; i < S.n; i++){
          ...
          const mesh = new THREE.Mesh(this.photonGeo, new THREE.MeshBasicMaterial({
            ...
            blending: THREE.AdditiveBlending, depthWrite: false }));
          mesh.position.set(this._owner.x, 1.1, this._owner.z); World.scene.add(mesh);
          this.photons.push({ x: this._owner.x, z: this._owner.z, y: 1.1,
    ...
    if (tr > 0){
      ...
      if (Math.random() < 0.5){
        ...
        FX.particle(this._owner.x + Math.cos(a) * rr, 1.0, this._owner.z + Math.sin(a) * rr,
    ...
    // 旋转相阵：环绕节点接触杀伤
    const ro = this._owner.progress.w('rotor');
    this.rotorT2 += dt;
    if (ro > 0){
      ...
      for (let i = 0; i < this.rotorNodes.length; i++){
        ...
        if (i < S.n){
          ...
          const rx = this._owner.x + Math.cos(a) * S.r, rz = this._owner.z + Math.sin(a) * S.r;
    ...
    // 太空雷阵：周期布设，敌近即爆
    const mn = this._owner.progress.w('mine');
    if (mn > 0){
      this.cd.mine -= dt;
      if (this.cd.mine <= 0){
        ...
        for (let i = 0; i < S.n; i++){
          ...
          slot.alive = true; slot.x = this._owner.x + Math.cos(a) * rr; slot.z = this._owner.z + Math.sin(a) * rr;
    ...
    // 纳米修复：周期治疗
    const na = this._owner.progress.w('nano');
    if (na > 0){
      this.cd.nano -= dt;
      if (this.cd.nano <= 0){
        const S = this.TABLE.nano[na - 1];
        this.cd.nano = S.cd * rm;
        this._owner.heal(S.hp);
        FX.cross(this._owner.x, this._owner.z, 0x5dff9b);
        FX.ring(this._owner.x, this._owner.z, 0x5dff9b, 4, 0.4);
        for (let k = 0; k < 6; k++){
          const a = Math.random() * Util.TAU;
          FX.particle(this._owner.x + Math.cos(a) * 2, 1.2, this._owner.z + Math.sin(a) * 2,
    ...
    /* ---------- E 新武器：径向脉冲 / 锋芒立场 / 蓄能长矛 ---------- */

    // 径向脉冲：周期向四周喷环形弹幕
    const rdl = this._owner.progress.w('radial');
    if (rdl > 0){
      this.cd.radial -= dt;
      if (this.cd.radial <= 0){
        const S = this.TABLE.radial[rdl - 1];
        this.cd.radial = S.cd * rm;
        const off = this.t * 0.7;
        for (let i = 0; i < S.n; i++){
          const a = i / S.n * Util.TAU + off;
          const crit = this.rollCrit();
          Bullets.fire(this._owner.x + Math.sin(a) * 1.1, this._owner.z + Math.cos(a) * 1.1,
            a, 50, S.dmg * (crit ? this.critMul() : 1),
            { crit, color: 0x9dffd6, scale: 0.8, life: 1.3 });
        }
        FX.ring(this._owner.x, this._owner.z, 0x9dffd6, 3.2, 0.3);
        Audio2.radialShot();
      }
    }

    // 锋芒立场：反射敌弹 + 周期灼烧近身敌人
    const rfl = this._owner.progress.w('reflect');
    if (rfl > 0){
      const S = this.TABLE.reflect[rfl - 1];
      this.reflectMesh.visible = true;
      this.reflectMesh.position.set(this._owner.x, 0.12, this._owner.z);
      const k = 1 + Math.sin(this.t * 4) * 0.06;
      this.reflectMesh.scale.setScalar(S.r * k);
      this.reflectMesh.material.opacity = 0.32 + Math.sin(this.t * 5) * 0.1;
      // 弹开穿入力场的敌弹，化为玩家子弹射回
      const ep = Bullets.ePool;
      const rr = S.r * S.r;
      for (let i = ep.active.length - 1; i >= 0; i--){
        const b = ep.active[i];
        if (Util.dist2(b.x, b.z, this._owner.x, this._owner.z) < rr){
          const dir = Math.atan2(b.vx, b.vz);
          const crit = this.rollCrit();
          Bullets.fire(this._owner.x + Math.sin(dir) * 1.0, this._owner.z + Math.cos(dir) * 1.0,
            dir, 64, S.dmg * (crit ? this.critMul() : 1),
            { crit, color: 0x8ff0ff, scale: 0.8, life: 1.2, pierce: 1 });
          ep.release(b);
          FX.burst(b.x, b.z, 0x8ff0ff, 2, 2, 1);
        }
      }
      // 周期灼烧近身敌人
      this.reflectTick -= dt;
      if (this.reflectTick <= 0){
        this.reflectTick = S.tick;
        const hits = Enemies.queryHit(this._owner.x, this._owner.z, S.r);
        for (const e of hits){
          const crit = this.rollCrit();
          Enemies.damage(e, S.dmg * (crit ? this.critMul() : 1), crit, e.x, e.z);
    ...
    } else if (this.reflectMesh) this.reflectMesh.visible = false;

    // 蓄能长矛：蓄力后射出穿透重矛
    const lcl = this._owner.progress.w('lance');
    if (lcl > 0){
      const S = this.TABLE.lance[lcl - 1];
      if (this.lanceCharging){
        this.lanceChargeT -= dt;
        this.lanceAim = T ? Math.atan2(T.x - this._owner.x, T.z - this._owner.z) : this._owner.yaw;
        // 蓄力指示：长矛沿瞄准方向显形，不透明度随蓄力增长
        this.lanceMesh.visible = true;
        this.lanceMesh.position.set(this._owner.x, 1.1, this._owner.z);
        this.lanceMesh.rotation.set(0, this.lanceAim, 0);
        this.lanceMesh.scale.set(1, 1, 28);
        const tp = 1 - Util.clamp(this.lanceChargeT / S.charge, 0, 1);
        this.lanceMesh.material.opacity = 0.15 + tp * 0.6;
        if (this.lanceChargeT <= 0){
          this.lanceCharging = false;
          this.cd.lance = S.cd * rm;
          const crit = this.rollCrit();
          const dmg = S.dmg * (crit ? this.critMul() : 1);
          Bullets.fire(this._owner.x + Math.sin(this.lanceAim) * 1.4, this._owner.z + Math.cos(this.lanceAim) * 1.4,
            this.lanceAim, S.spd, dmg,
            { pierce: S.pierce, crit, color: 0xbfe9ff, scale: 1.7, life: 1.5, long: 2.6 });
          FX.ring(this._owner.x + Math.sin(this.lanceAim) * 1.6, this._owner.z + Math.cos(this.lanceAim) * 1.6, 0xbfe9ff, 2.6, 0.22);
          FX.burst(this._owner.x + Math.sin(this.lanceAim) * 2, this._owner.z + Math.cos(this.lanceAim) * 2, 0xbfe9ff, 6, 5, 1.2);
          Audio2.lance();
        }
      } else {
        this.cd.lance -= dt;
        if (this.cd.lance <= 0 && T){ this.lanceCharging = true; this.lanceChargeT = S.charge; }
        else if (this.lanceMesh) this.lanceMesh.visible = false;
      }
    } else if (this.lanceMesh) this.lanceMesh.visible = false;

    /* ---------- E+ 新武器：脉冲星斩舰炮 / 攻城轨道炮 / 离子散射 / 离子干扰 ---------- */

    // 脉冲星斩舰炮：蓄能后射出贯穿脉冲束，沿途敌人高伤并被击退
    const plr = this._owner.progress.w('pulsar');
    if (plr > 0){
      const S = this.TABLE.pulsar[plr - 1];
      if (this.pulsarCharging){
        this.pulsarChargeT -= dt;
        this.pulsarAim = T ? Math.atan2(T.x - this._owner.x, T.z - this._owner.z) : this._owner.yaw;
        if (this.pulsarChargeT <= 0){
          this.pulsarCharging = false;
          this.cd.pulsar = S.cd * rm;
          const crit = this.rollCrit();
          const dmg = S.dmg * (crit ? this.critMul() : 1);
          Bullets.fire(this._owner.x + Math.sin(this.pulsarAim) * 1.4, this._owner.z + Math.cos(this.pulsarAim) * 1.4,
            this.pulsarAim, S.spd, dmg,
            { pierce: 999, crit, color: 0x7df0ff, scale: 1.9, life: 0.85, long: 3.0 });
          FX.ring(this._owner.x + Math.sin(this.pulsarAim) * 1.8, this._owner.z + Math.cos(this.pulsarAim) * 1.8, 0x7df0ff, 3, 0.3);
          FX.burst(this._owner.x + Math.sin(this.pulsarAim) * 2.2, this._owner.z + Math.cos(this.pulsarAim) * 2.2, 0x7df0ff, 8, 6, 1.2);
          Audio2.pulsarShot();
        }
      } else {
        this.cd.pulsar -= dt;
        if (this.cd.pulsar <= 0 && T){ this.pulsarCharging = true; this.pulsarChargeT = S.charge; }
      }
    }

    // 攻城轨道炮：蓄能后锁定最远敌人，落点大范围爆破
    const sg = this._owner.progress.w('siege');
    if (sg > 0){
      const S = this.TABLE.siege[sg - 1];
      if (this.siegeCharging){
        this.siegeChargeT -= dt;
        if (this.siegeChargeT <= 0){
          this.siegeCharging = false;
          this.cd.siege = S.cd * rm;
          let bx = this._owner.x, bz = this._owner.z + 50, best = -1;
          const ea = Enemies.pool.active;
          for (let _i = 0; _i < ea.length; _i++){
            const _e = ea[_i]; if (!_e.alive) continue;
            const _d = Util.dist2(this._owner.x, this._owner.z, _e.x, _e.z);
            if (_d > best){ best = _d; bx = _e.x; bz = _e.z; }
          }
          const crit = this.rollCrit();
          const dmg = S.dmg * (crit ? this.critMul() : 1);
          FX.ring(bx, bz, 0xff8a3d, S.r, 0.45);
          FX.explode(bx, bz, 0xff8a3d, 2.0);
          Enemies.splash(bx, bz, S.splash, dmg);
          if (Boss.active && Boss.hitTest(bx, bz, S.splash)) Boss.damage(dmg, crit, bx, bz);
          World.shake(1.2, 0.3);
          Audio2.siege();
        }
      } else {
        this.cd.siege -= dt;
        if (this.cd.siege <= 0 && T){ this.siegeCharging = true; this.siegeChargeT = S.charge; }
      }
    }

    // 离子散射炮：一次扇形发射多枚高速离子弹
    const sc = this._owner.progress.w('scatter');
    if (sc > 0){
      this.cd.scatter -= dt;
      if (this.cd.scatter <= 0 && T){
        const S = this.TABLE.scatter[sc - 1];
        this.cd.scatter = S.cd * rm;
        const base = Math.atan2(T.x - this._owner.x, T.z - this._owner.z);
        const crit = this.rollCrit();
        const spread = 0.55;
        for (let i = 0; i < S.n; i++){
          const a = base + (i / (S.n - 1) - 0.5) * spread;
          Bullets.fire(this._owner.x + Math.sin(a) * 1.1, this._owner.z + Math.cos(a) * 1.1,
            a, S.spd, S.dmg * (crit ? this.critMul() : 1),
            { crit, color: 0xa8e6ff, scale: 0.7, life: 1.1 });
        }
        FX.ring(this._owner.x, this._owner.z, 0xa8e6ff, 2.2, 0.2);
        Audio2.scatterShot();
      }
    }

    // 离子干扰炮：三连发离子弹，命中使敌人减速
    const io = this._owner.progress.w('ion');
    if (io > 0){
      this.cd.ion -= dt;
      if (this.cd.ion <= 0 && T){
        const S = this.TABLE.ion[io - 1];
        this.cd.ion = S.cd * rm;
        const base = Math.atan2(T.x - this._owner.x, T.z - this._owner.z);
        const crit = this.rollCrit();
        const spread = 0.22;
        for (let i = 0; i < S.n; i++){
          ...
          Bullets.fire(this._owner.x + Math.sin(a) * 1.1, this._owner.z + Math.cos(a) * 1.1,
            a, S.spd, S.dmg * (crit ? this.critMul() : 1),
            { crit, color: 0xc9a8ff, scale: 0.65, life: 1.0, slow: S.slow, slowK: S.slowK });
        }
        Audio2.ionShot();
      }
    }
  ...
  pickTargets(n){
    ...
    for (let i = 0; i < list.length; i++){
      const e = list[i];
      const d = Util.dist2(this._owner.x, this._owner.z, e.x, e.z);
  ...
  /** 贯穿光束：沿射线打到所有敌人 */
  beam(target, S, lv){
    const dir = Math.atan2(target.x - this._owner.x, target.z - this._owner.z);
    const len = 54;
    ...
    for (let t = 1; t < len; t += step){
      const px = this._owner.x + dx * t, pz = this._owner.z + dz * t;
    ...
    if (b){
      b.life = 1;
      b.mesh.visible = true;
      b.mesh.position.set(this._owner.x, 1.0, this._owner.z);
    ...
    FX.burst(this._owner.x + dx * 1.6, this._owner.z + dz * 1.6, 0x9df6ff, 4, 4, 1);
  },
};

/* ============ 双人支持：按玩家实例化的武器集工厂 ============
   Weapons 本体保留为 P1 的武器集（所有现存 Weapons.xxx 调用等价于 P1）。
   makeWeapons(owner) 用 Object.create(Weapons) 继承全部方法，
   但用全新的状态字段（冷却 / 实体池 / 网格引用）避免与 P1 共享可变状态。*/
function makeWeapons(owner){
  const w = Object.create(Weapons);
  w._owner = owner;
  w.currentTarget = null; w.t = 0;
  w.cd = {}; for (const k in Weapons.cd) w.cd[k] = 0;
  w.beams = []; w.bolts = []; w.blades = []; w.drones = []; w.meteors = [];
  w.saws = []; w.pulses = []; w.blackholes = []; w.photons = []; w.rotorNodes = []; w.minePool = [];
  w.auraMesh = null; w.auraDisc = null; w.auraCyl = null;
  w.beamGroup = null; w.boltGroup = null; w.bladeGroup = null; w.sawGroup = null; w.rotorGroup = null; w.droneGroup = null;
  w.photonGeo = null; w.mineGeo = null; w.tractorMesh = null; w.reflectMesh = null; w.lanceMesh = null;
  w.auraT = 0; w.orbitT = 0; w.droneT = 0; w.novaT = 0; w.rotorT2 = 0; w.reflectTick = 0;
  w.lanceCharging = false; w.lanceChargeT = 0; w.lanceAim = 0;
  w.pulsarCharging = false; w.pulsarChargeT = 0; w.pulsarAim = 0;
  w.siegeCharging = false; w.siegeChargeT = 0; w.siegeAim = 0;
  return w;
}
const Weapons2 = makeWeapons(Player2);
// 装配：P1 武器集归属 Player，P2 武器集归属 Player2
Weapons._owner = Player;
Player.weapons = Weapons;
Player2.weapons = Weapons2;
...
const Wingmen = {
  ...
  SPEC: {
    ...
    cruiser:  { name:'护卫舰',   color:0xffb24d,              cd:1.55, dmg:31, spd:18, mesh:'enemy_charger', s:1.05 },
    interceptor:{ name:'拦截机', color:0x4dd2ff,              cd:0.34, dmg:7,  spd:30, mesh:'wing_a',       s:0.80 },
    sentinel: { name:'哨戒舰',   color:0x9d8bff,              cd:1.10, dmg:17, spd:22, mesh:'wing_b',       s:0.95 },
    spitfire: { name:'喷火僚机', color:0xff7a4d,              cd:0.40, dmg:9,  spd:32, mesh:'wing_a',       s:0.82 },
    bulwark:  { name:'壁垒僚机', color:0xffd24a,              cd:1.40, dmg:26, spd:20, mesh:'wing_b',       s:1.05 },
  },
  ...
  update(dt){
    ...
    for (let i = 0; i < n; i++){
      ...
      // 自主索敌：比玩家更宽的视野，体现「自由」；优先打 BOSS / 精英，再就近
      let tgt = (Boss.active && !Boss.entering) ? Boss.asTarget() : null;
      if (!tgt){
        const _ea = Enemies.pool.active; let _bd = 44 * 44, _best = null, _elite = null;
        for (let _k = 0; _k < _ea.length; _k++){
          const _e = _ea[_k]; if (!_e.alive) continue;
          const _dd = Util.dist2(w.x, w.z, _e.x, _e.z);
          if (_dd > _bd) continue;
          if (_e.elite){ if (!_elite) _elite = _e; }
          else if (!_best) _best = _e;
        }
        tgt = _elite || _best;
      }
      ...
      if (w.type === 'medic'){
        ...
        if (w.cd <= 0){
          ...
          } else if (w.type === 'cruiser'){
            // 护卫舰：双联重型穿透能量弹
            for (let s = 0; s < 2; s++){
              const off = (s - 0.5) * 0.18;
              Bullets.fire(w.x + Math.sin(dir + off) * 1.0, w.z + Math.cos(dir + off) * 1.0,
                dir + off, w.spec.spd * 0.8, w.spec.dmg * mul,
                { crit, y: 1.4, color: 0xffb24d, scale: 1.3, life: 1.4, pierce: 2 });
            }
          } else {
            Bullets.fire(w.x, w.z, dir, w.spec.spd, w.spec.dmg * mul,
              { crit, y: 1.4, color: w.spec.color, scale: 0.9, life: 0.8 });
          }
